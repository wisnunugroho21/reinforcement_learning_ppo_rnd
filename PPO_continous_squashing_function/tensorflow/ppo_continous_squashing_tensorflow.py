...
class Actor_Model(Model):
  def __init__(self, state_dim, action_dim):
    ...
    self.mean     = Dense(action_dim, activation='linear')
    self.log_std  = Dense(action_dim, activation='linear')
...
class Memory():
    ...
    def get_all_items(self):
        ...
        preactions = tf.constant(self.preactions, dtype = tf.float32)
        ...
        return tf.data.Dataset.from_tensor_slices((states, preactions, rewards, dones, next_states))      

    def save_eps(self, state, preaction, reward, done, next_state):
        self.rewards.append(reward)
        self.states.append(state)
        self.preactions.append(preaction)
...
class Agent():  
    ...
    def save_eps(self, state, preaction, reward, done, next_state):
        self.memory.save_eps(state, preaction, reward, done, next_state)

    # Loss for PPO  
    def get_loss(self, action_mean, log_action_std, values, old_action_mean, old_log_action_std, old_values, next_values, preactions, rewards, dones):   
        # Don't use old value in backpropagation
        Old_values      = tf.stop_gradient(old_values)

        action_std      = tf.math.exp(log_action_std)
        old_action_std  = tf.math.exp(old_log_action_std)
        ...
        logprobs        = self.distributions.logprob(action_mean, action_std, preactions)
        Old_logprobs    = tf.stop_gradient(self.distributions.logprob(old_action_mean, old_action_std, preactions))
    ...
    def act(self, state):
        ...
        action_std                  = tf.math.exp(action_std)
        
        # We don't need sample the action in Test Mode
        # only sampling the action in Training Mode in order to exploring the actions
        if self.is_training_mode:
            # Sample the action
            preaction   = self.distributions.sample(action_mean, action_std)
            action      = tf.math.tanh(preaction) # Squashing by tanh will make action bound in range -1 to 1
        else:
            action      = tf.math.tanh(action_mean)

        return tf.squeeze(action), tf.squeeze(preaction)

    # Get loss and Do backpropagation
    @tf.function
    def training_ppo(self, states, preactions, rewards, dones, next_states):        
        with tf.GradientTape() as tape:
            action_mean, log_action_std         = self.actor(states)
            values                              = self.critic(states)
            old_action_mean, old_log_action_std = self.actor_old(states)
            old_values                          = self.critic_old(states)
            next_values                         = self.critic(next_states)

            loss = self.get_loss(action_mean, log_action_std, values, old_action_mean, old_log_action_std, old_values, next_values, preactions, rewards, dones)
    ...
    def update_ppo(self):        
        ...
        for _ in range(self.PPO_epochs):       
            for states, preactions, rewards, dones, next_states in self.memory.get_all_items().batch(batch_size):
                self.training_ppo(states, preactions, rewards, dones, next_states)
...
def run_episode(env, agent, state_dim, render, training_mode, t_updates, n_update):
    ...
    while not done:
        action, preaction           = agent.act(state)
        preaction                   = preaction.numpy()
        ...
        if training_mode:
            agent.save_eps(state.tolist(), preaction, reward, float(done), next_state.tolist()) 
